...
CREATE TABLE IF NOT EXISTS public.document_embeddings (
    ...
    founder_id      UUID REFERENCES public.founders(id) ON DELETE CASCADE, -- NULL for global news
...
CREATE POLICY "embeddings_select_own" ON public.document_embeddings
    FOR SELECT USING (auth.uid() = founder_id OR founder_id IS NULL);
...
AS $$
    ...
    -- Match either the founder's own docs OR global system docs (NULL founder_id)
    WHERE (founder_id = founder_uuid OR founder_id IS NULL)
